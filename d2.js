/**
 * Script for day 2 in advent of code calendar
 * http://adventofcode.com/2017/day/2
 * 
 * Quick and dirty , just get the result.
 * 
 * To run this code, run the command node d1.js
 * 
 */
...
class Puzzle {
  ...
  constructor() {
    ...
    this.inputString = `4168	3925	858	2203	440	185	2886	160	1811	4272	4333	2180	174	157	361	1555
...
37	1069	166	1037	172	258	1071	90	497	1219	145	1206	143	153	1067	510`
  ...
  getAnswer() {
    ...
    return `The answer is ... ${theAnswerIs_Attention_dot_dot_dot}`
  }

  theSolver( inputVal ) {

    // Clean input to have cleaned  mutliple array
    let inputArrayMultiline = inputVal.split("\n")
    let inputFormatted = inputArrayMultiline.map( 
      ( element ) => { 
        let mixedDigitArray =  element.split('\t') 
        let cleanArray = mixedDigitArray.filter( (element) => {return element != ''})
        return cleanArray
      }
    )
    ...
    for( var i = 0; i < inputFormatted.length; i++ ){

      let minVal =  1000000000
      let maxVal =  0
      let currentLine = inputFormatted[i]
      
      for( var j = 0; j < currentLine.length; j++ ){
        let currentVal = parseInt(currentLine[j])
  ...
  theChecker() {
    ...
    let test1 = this.theSolver(this.inputTest1)
    if( test1 === this.expectTest1 ) {
      console.log("> test 1 OK :)");
    }else{
      console.log(`> test 1 Not OK :(  => Got ${test1} , expecting ${this.expectTest1}`);
    }
